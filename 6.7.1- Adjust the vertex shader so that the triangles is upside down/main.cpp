...
int main(void)
{
	...
	Shader shader("Shaders/6.7.1-Shader.vert", "Shaders/6.7.1-Shader.frag");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		GLuint ourColor_LOCATION = glGetUniformLocation(shader.getProgramID(), "ourColor");
		shader.Use();
		float value = (sin(glfwGetTime()) / 2) + 0.5f;
		glUniform4f(ourColor_LOCATION, 1.0f, value, 0.25f, 1.0f);
