...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	{
		mixValue += 0.1f;
		direction = glm::vec3(direction.x, direction.y, direction.z+0.5f);
	}
	...
	{
		direction = glm::vec3(direction.x - 0.5f, direction.y, direction.z);
	}
...
{	
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 view(1.0f);
		direction = glm::vec3(direction.x-0.02f, direction.y, direction.z-0.05f);
